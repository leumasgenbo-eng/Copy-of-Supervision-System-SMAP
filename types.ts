...
export interface StudentData {
    ...
    exerciseScores?: Record<string, Record<string, number>>; // Subject -> ExerciseID -> Score
...
export interface ExerciseAssessmentRecord {
    ...
    pupilsDefaulters: string[]; // IDs
    pupilsMarked: string[]; // IDs
    pupilsMissing: string[]; // IDs
    natureOfQuestions: number; // 1-5 scale
    bloomsTaxonomy: string[]; // Multi-select
    devNumberLink: string;
    handwritingLegibility: number; // 0-10
    handwritingLegibilityComment: string;
    handwritingClarity: number; // 0-10
    handwritingClarityComment: string;
    spellWellCount: number;
    goodAppearance: boolean;
    facilitatorPreparedTestItems: boolean;
    confirmedWithPupil: string; // Name/ID
...
export interface GlobalSettings {
  ...
  exerciseAssessments?: Record<string, ExerciseAssessmentRecord[]>; // Class -> List
