...
const DaycareReportCard: React.FC<DaycareReportCardProps> = ({ student, settings, onSettingChange, onStudentUpdate, schoolClass, totalStudents }) => {
  ...
  const handleSharePDF = async () => {
    ...
    if (!originalElement) { alert("Report element not found."); setIsGenerating(false); return; }
    // @ts-ignore
    if (typeof window.html2pdf === 'undefined') { alert("PDF generator not ready."); setIsGenerating(false); return; }
    ...
    const opt = { margin: 0, filename: `${student.name.replace(/\s+/g, '_')}_Daycare_Report.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, windowWidth: 794 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
    try {
        // @ts-ignore
  ...
  return (
    <div id={`daycare-report-${student.id}`} className="bg-white p-6 max-w-[210mm] mx-auto h-[296mm] border border-gray-200 shadow-sm print:shadow-none print:border-none page-break relative group flex flex-col box-border font-sans">
       ...
       <div className="text-center mb-4">
          <EditableField value={settings.schoolName} onChange={(v) => onSettingChange('schoolName', v)} className="text-center font-black w-full bg-transparent text-3xl text-blue-900 tracking-widest uppercase leading-tight mb-0" multiline rows={1} />
          ...
          <div className="flex justify-center gap-4 text-[10px] font-semibold text-gray-800 mb-2 uppercase">
          ...
          <h2 className="text-lg font-bold text-red-700 uppercase leading-tight">STANDARD BASED CURRICULUM, LEARNER’S PERFORMANCE REPORT</h2>
       </div>
       ...
       <div className="grid grid-cols-2 gap-x-8 gap-y-2 mb-4 text-sm font-semibold border-b-2 border-gray-800 pb-4">
          ...
          <div className="flex items-end gap-2"><span>Term:</span><EditableField value={settings.termInfo} onChange={(v) => onSettingChange('termInfo', v)} className="w-24 text-center border-b border-dotted border-gray-600" /></div>
       ...
       <div className="flex-1 mb-4 flex flex-col gap-4">
           <div className="border border-gray-800">
               <div className="bg-gray-200 font-bold text-xs uppercase border-b border-gray-800 p-1 text-center">Learning Areas / Subjects</div>
               <div className="flex font-bold text-[10px] uppercase border-b border-gray-800 bg-gray-50">
                   ...
                   {coreRanges.map(range => (<div key={range.grade} className="w-10 p-2 text-center border-r border-gray-600 last:border-0">{range.grade}</div>))}
               </div>
               {coreSubjectsDisplay.map(sub => {
                   ...
                   return (
                       <div key={sub.subject} className="flex border-b border-gray-400 text-xs last:border-0">
                           ...
                           {coreRanges.map(range => (<div key={range.grade} className="w-10 p-2 text-center border-r border-gray-600 last:border-0 flex justify-center items-center">{grade === range.grade ? '✔' : ''}</div>))}
                       </div>
                   );
               })}
           </div>
           ...
           <div className="border border-gray-800">
                <div className="bg-gray-100 p-1 font-bold text-xs border-b border-gray-400 text-center uppercase">Assessment on Social, Physical and Cultural Development</div>
                <div className="flex font-bold text-[10px] uppercase border-b border-gray-800 bg-gray-50">
                   <div className="flex-1 p-2 border-r border-gray-600">Observed Indicator</div>
                   {indicatorRanges.map(range => (<div key={range.grade} className="w-10 p-2 text-center border-r border-gray-600 last:border-0">{range.grade}</div>))}
               </div>
               {indicatorsDisplay.map(skill => {
                   ...
                   return (
                        <div key={skill} className="flex border-b border-gray-400 text-xs last:border-0">
                           ...
                           {indicatorRanges.map(range => (<div key={range.grade} className="w-10 p-1 text-center border-r border-gray-600 last:border-0 flex justify-center items-center font-bold">{rating === range.grade ? '✔' : ''}</div>))}
                       </div>
                   );
               })}
           </div>
       ...
       <div className="text-xs font-semibold space-y-3">
           <div className="flex items-center gap-2">
               ...
               <div className="flex items-center border-b border-dotted border-gray-600 px-2">
                    <EditableField value={student.attendance || "0"} onChange={(v) => onStudentUpdate(student.id, 'attendance', v)} className="w-8 text-center" />
                    <span> OUT OF </span>
               ...
               <EditableField value={student.promotedTo || ""} onChange={(v) => onStudentUpdate(student.id, 'promotedTo', v)} className="flex-1 border-b border-dotted border-gray-600 uppercase" />
           </div>
           ...
           <div className="flex items-start gap-2"><span className="whitespace-nowrap">OVERALL REMARK:</span><EditableField value={student.overallRemark || ""} onChange={(v) => onStudentUpdate(student.id, 'finalRemark', v)} multiline className="flex-1 border-b border-dotted border-gray-600 leading-tight" /></div>
           <div className="flex justify-between items-end mt-8 pt-4">
               ...
               <div className="w-5/12 text-center"><div className="border-b border-black h-8"></div><p>SIGN (H/T)</p></div>
           </div>
       </div>
    </div>
  );
